...
impl ValidationEngine {
    pub fn run_all() -> Result<Vec<ValidationCheck>> {
        ...
        println!(
            "{}",
            "            PROJECT OBSIDIAN — SYSTEM HEALTH & VALIDATION MATRIX               "
                .cyan()
        );
        println!(
            "{}",
        ...
        let cmd = format!(
            ". '{}'; Test-ObsidianHealth | ConvertTo-Json",
            val_p.display()
        );
        ...
        if all_healthy {
            println!(
                "{}",
                "[V] ALL CRITICAL SUBSYSTEMS REPORT HEALTHY. ZERO BREAKAGE CONFIRMED."
                    .green()
                    .bold()
            );
        } else {
            println!(
                "{}",
                "[!] Warning: One or more components reported warnings. Review details above."
                    .yellow()
                    .bold()
            );
        }
